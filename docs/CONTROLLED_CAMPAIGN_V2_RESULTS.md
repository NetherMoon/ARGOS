...
The report package contains the complete original/mixed result tables, all candidate selections/confirmation evidence, all failures, bank identity/runtime table, prediction summaries, source analysis, closure and audit receipts, and a SHA256 inventory. Raw banks, simulator inputs/output traces, per-method states and immutable completion seals remain in `runs/campaigns/controlled_development_v2/`. No `.env` file, trained model, reserved seed value or raw multi-gigabyte cache is copied into this documentation package.
