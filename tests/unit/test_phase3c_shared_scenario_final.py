...
from argos.experimental_sa.phase3c.planning import (
    BLOCK_SIZE, NAIVE_CALLS_PER_TRAJECTORY, PANELS, PANEL_SIZE,
    SHARED_CALLS_PER_TRAJECTORY, TRANSITIONS, _generate_seed_plan,
    create_plan, validate_seed_plan,
)
...
class Domain:
    def validate(self, _value): pass
    def project(self, value):
        x = np.asarray(value, dtype=float); x[0] = np.clip(x[0], .4, .6); x[1] = np.clip(x[1], .01, .2)
        x[2:] = np.maximum(x[2:], .01); x[2:] /= x[2:].sum(); return tuple(x)


class Contract:
    def evaluate(self, monetary, p90, pj):
        value = float(monetary + p90 + sum(pj))
        return SimpleNamespace(to_dict=lambda: {"M_RSR": monetary, "Ctrack": p90,
                                                 "CQoS": sum(pj), "Cfull": value,
                                                 "qos_terms": tuple(pj)})


class PanelEvaluator:
    def __init__(self, fail_once=None, infeasible=False, increasing=False):
        self.fail_once = set(fail_once or []); self.failed = set(); self.calls = []
        self.infeasible = infeasible; self.increasing = increasing
    def evaluate_panel(self, *, params, evaluation_id, panel_index, panel_seeds, runtime_seed):
        ...
        if evaluation_id in self.fail_once and evaluation_id not in self.failed:
            self.failed.add(evaluation_id); raise RuntimeError("synthetic interruption")
        raws=[]
        for seed in panel_seeds:
            p90=.4 if self.infeasible else .2
            raws.append({"status":"COMPLETE","arrival_seed":seed,"runtime_seed":runtime_seed,
                         "p90":p90,"mean_tracking":p90,"M_RSR":1.0 + (10.0 * evaluation_id if self.increasing else 0.0),"Pj":[.01]*4,
                         "evidence_counts":[10]*4,"grid_signal_hash":"grid",
                         "initial_job_table_hash":f"jobs-{seed}","target_trace_hash":"target"})
        return {"raw_results":raws,"performed_calls":3}


def draws(count):
    return [{"iteration":i,"p_standard_normal":.1,"r_standard_normal":.1,
             "weight_standard_normals":[0.,0.,0.,0.],"metropolis_uniform":.99}
            for i in range(1,count+1)]


def run_small(tmp_path, evaluator, transitions=4, block_size=2):
    panels=[[100+i*3+j for j in range(3)] for i in range((transitions+block_size-1)//block_size)]
    return run_shared3_trajectory(initial=(.47,.08,.25,.25,.25,.25),domain=Domain(),
                                  evaluator=evaluator,contract=Contract(),job_names=JOBS,
                                  output=tmp_path,run_id="test",panel_schedule=panels,
                                  runtime_seed=999,search_draws=draws(transitions),
                                  transitions=transitions,block_size=block_size,temperature=10,
                                  cooling_rate=.95,steps=(.01,.01,.02))


def test_seed_plan_is_deterministic_unique_and_disjoint():
    prior={1,2,3}; first=_generate_seed_plan(prior); second=_generate_seed_plan(prior)
    assert first==second; validate_seed_plan(first,prior)
...
def test_full_plan_has_exact_frozen_budgets(tmp_path):
    ...
    import csv
    def read(name):
        with (experiment/name).open(newline="",encoding="utf8") as stream: return list(csv.DictReader(stream))
    calls=read("optimization_call_plan.csv"); assessment=read("assessment_run_plan.csv")
    assert len(calls)==6884; assert sum(x["method"]=="naive" for x in calls)==1604
    assert sum(x["method"]=="shared3" for x in calls)==5280; assert len(assessment)==200
...
def test_shared_engine_uses_three_scenarios_cache_and_switch_reevaluation(tmp_path):
    evaluator=PanelEvaluator(); result=run_small(tmp_path,evaluator)
    assert result.simulator_calls==18; assert result.panel_evaluations==6
    rows=[json.loads(x) for x in (tmp_path/"iterations.jsonl").read_text().splitlines()]
    assert len(rows)==5; assert len(list((tmp_path/"panel_events").glob("*.json")))==1
...
def test_panel_all_feasible_requires_three_valid_feasible_scenarios():
    evaluator=PanelEvaluator(infeasible=True); response=evaluator.evaluate_panel(params=[1],evaluation_id=0,panel_index=0,panel_seeds=[1,2,3],runtime_seed=9)
    item=aggregate_panel(params=[1],candidate_id="x",evaluation_id=0,panel_index=0,panel_seeds=[1,2,3],raw_results=response["raw_results"],contract=Contract(),job_names=JOBS)
    assert item["panel_pass_count"]==0; assert not item["panel_all_feasible"]
...
def test_resume_inside_panel_is_deterministic(tmp_path):
    ...
    try: run_small(tmp_path,evaluator)
...
def test_compact_zip_excludes_scratch(tmp_path):
    experiment=tmp_path/"phase3c"; experiment.mkdir(); (experiment/"manifest.json").write_text("{}")
    scratch=experiment/"assessment/cells/x/scratch"; scratch.mkdir(parents=True); (scratch/"big.csv").write_text("x")
    (scratch.parent/"result.json").write_text("{}")
    target=build_zip(experiment)
    import zipfile
    with zipfile.ZipFile(target) as archive: names=archive.namelist()
    assert any(x.endswith("result.json") for x in names); assert not any(x.endswith("big.csv") for x in names)
...
def test_report_constructs_final_pre_argos_decision(tmp_path):
    ...
    for case in ["c005","c007"]:
        for role in ["starting", "replicate_1_naive", "replicate_1_shared3",
                     "replicate_2_naive", "replicate_2_shared3"]:
            ...
            for panel,count in [("A",6),("B",7),("COMBINED",13)]:
                item={"case":case,"candidate_role":role,"replicate":replicate,"method":method,
                      "panel":panel,"pass_count":count,"proposed_8_of_10_scenario_criterion":False,
                      "average_metric_feasible":True,"mean_p90":.25,"mean_Bloom_Pj":.03,"mean_Cfull":88.0}
                rows.append(item)
    summary=pd.DataFrame(rows)
    pairs=pd.DataFrame([{"case":case,"replicate":rep,"classification":"BOTH_PASS"}
                        for case in ["c005","c007"] for rep in [1,2]])
    stable=pd.DataFrame([{"case":case,"method":method,"s3_wins":1,"naive_wins":0,"ties":1}
                         for case in ["c005","c007"] for method in ["naive","shared3"]])
    optimization=pd.DataFrame([{"method":method,"simulator_calls":401 if method=="naive" else 1320,
                                "wall_clock_seconds":1.0}
                               for _case in ["c005","c007"] for _rep in [1,2]
