...
from argos.experimental_sa.phase3.scenarios import Scenario, VaryingArrivalScenarioPolicy, read_search_draws

from .engine import run_shared3_trajectory
from .evaluator import SharedPanelEvaluator
from .planning import (
    BLOCK_SIZE, CASES, METHODS, NAIVE_EVALUATIONS, OPTIMIZATION_RUNTIME_SEED,
    PANELS, REPLICATES, SHARED_CALLS_PER_TRAJECTORY, TRANSITIONS,
    create_plan, freeze_contract, verify_frozen_contract,
)
...
def run_named_trajectory(root: Path, experiment: Path, name: str) -> dict:
    ...
    draws = read_search_draws(experiment / f"search_draw_schedule/replicate_{replicate}/{label}.csv")
    ...
    if method == "naive":
        ...
        result_obj = run_trajectory(
            initial=(case["Pbar"], case["R"], *case["weights"]), domain=domain,
            evaluator=evaluator, contract=ObjectiveContract(root),
            job_names=list(jobs.all_jobs.values()), output=output,
            run_id=f"replicate_{replicate}_{case_id}_naive", scenario_policy=policy,
            search_draws=draws, transitions=TRANSITIONS, **_settings(root, domain),
        )
        ...
        result_obj = run_shared3_trajectory(
            ...
            runtime_seed=OPTIMIZATION_RUNTIME_SEED, search_draws=draws,
            transitions=TRANSITIONS, block_size=BLOCK_SIZE, **_settings(root, domain),
        )
        result = result_obj.to_dict()
    ...
    metadata = {
        "trajectory": name, "method": method,
        "invocations": int(previous.get("invocations", 0)) + 1,
        "wall_clock_seconds_accumulated": float(previous.get("wall_clock_seconds_accumulated", 0.0)) + (time.perf_counter() - started),
...
def run_smoke(root: Path, experiment: Path) -> dict:
    ...
    common = dict(
        initial=(case["Pbar"], case["R"], *case["weights"]), domain=domain,
        evaluator=evaluator, contract=ObjectiveContract(root), job_names=list(jobs.all_jobs.values()),
        output=output, run_id="phase3c_real_smoke", runtime_seed=OPTIMIZATION_RUNTIME_SEED,
        block_size=2, **_settings(root, domain),
    )
    run_shared3_trajectory(panel_schedule=plan["smoke"]["panel_seeds"][:1],
                           search_draws=draws[:2], transitions=2, **common)
    result = run_shared3_trajectory(panel_schedule=plan["smoke"]["panel_seeds"],
                                    search_draws=draws, transitions=4, **common)
    ...
    summary = {
        ...
        "transitions": 4, "blocks": 2, "panel_switches": len(events),
        "panel_evaluations": result.panel_evaluations,
        "scenario_observations_per_panel": sorted({len(x["scenario_observations"]) for x in observations}),
        ...
        "accepted_count": sum(accepted), "rejected_count": len(accepted) - sum(accepted),
    ...
    if not (result.simulator_calls == 18 and result.panel_evaluations == 6 and len(events) == 1
    ...
    manifest["status"] = "READY_FOR_USER_RUN"; manifest["smoke"] = summary
...
def validate_optimization(root: Path, experiment: Path) -> None:
    ...
    for replicate in REPLICATES:
        for label, case in CASES:
            paths = {m: experiment / f"optimization/replicate_{replicate}/{case}_{m}" for m in METHODS}
            naive = [json.loads(x) for x in (paths["naive"] / "iterations.jsonl").read_text().splitlines()]
            shared = [json.loads(x) for x in (paths["shared3"] / "iterations.jsonl").read_text().splitlines()]
            ...
            hashes = {y["raw"]["initial_job_table_hash"] for x in shared_observations for y in x["scenario_observations"]}
            if len(hashes) != 120:
                raise ValueError("S3 does not contain 120 distinct arrival hashes")
            for row in shared:
                expected = plan["shared_panel_seeds"][int(row["panel_index"])]
                if row["panel_seeds"] != expected or row["panel_evaluation"]["panel_seeds"] != expected:
            ...
            for method, path in paths.items():
                ...
                if result["status"] == "EXECUTION_ERROR" or int(result["simulator_calls"]) != expected_calls:
                    raise ValueError("Phase 3C trajectory result/call count mismatch")
            grids = {x["raw"]["grid_signal_hash"] for x in naive}
            grids |= {y["raw"]["grid_signal_hash"] for x in shared_observations for y in x["scenario_observations"]}
            runtimes = {int(x["raw"]["runtime_seed"]) for x in naive}
            runtimes |= {int(y["raw"]["runtime_seed"]) for x in shared_observations for y in x["scenario_observations"]}
...
def build_selected_candidates(root: Path, experiment: Path) -> list[dict]:
    ...
    for label, case_id in CASES:
        ...
        rows.append({"case_label": label, "case": case_id, "candidate_role": "starting",
                     "replicate": 0, "method": "starting", "selection": "frozen_starting_candidate",
                     "sa_success": True, "optimization_candidate_id": case["candidate_id"],
                     "Pbar": case["Pbar"], "R": case["R"],
                     "weights": json.dumps(case["weights"], separators=(",", ":")),
                     "optimization_objective": "", "source_path": "Phase2B frozen starting candidate"})
        for replicate in REPLICATES:
            for method in METHODS:
                path = experiment / f"optimization/replicate_{replicate}/{case_id}_{method}/result.json"
                item, selection, success = _selected_from_result(path, method)
                rows.append({"case_label": label, "case": case_id,
                             "candidate_role": f"replicate_{replicate}_{method}",
                             "replicate": replicate, "method": method, "selection": selection,
                             "sa_success": success, "optimization_candidate_id": item["candidate_id"],
                             "Pbar": item["params"][0], "R": item["params"][1],
                             "weights": json.dumps(item["params"][2:], separators=(",", ":")),
                             "optimization_objective": item.get("panel_mean_Cfull", item["objective"]["Cfull"]),
...
def build_assessment_requests(experiment: Path, candidates: list[dict]) -> list[dict]:
    ...
    for row in _read_csv(experiment / "assessment_run_plan.csv"):
        ...
        requests.append({**row, "selection": candidate["selection"],
                         "sa_success": candidate["sa_success"],
                         "params": json.dumps([candidate["Pbar"], candidate["R"], *json.loads(candidate["weights"])], separators=(",", ":")),
...
def assessment_worker(root: Path, experiment: Path, request_path: Path) -> None:
    request = _read(request_path); output = experiment / "assessment/cells" / request["cell_id"]
    ...
    raw = response["raw"]; objective = ObjectiveContract(root).evaluate(raw["M_RSR"], raw["p90"], raw["Pj"]).to_dict()
    result = {**request, "params": json.loads(request["params"]), "raw": raw,
              "objective": objective, "feasibility": assess(raw, list(jobs.all_jobs.values()), objective["Cfull"])}
    _write(result_path, result); evaluator.finalize_iteration(0, Scenario(0, seed, seed), True)
...
def run_long(root: Path, experiment: Path, max_parallel_sa: int, assessment_workers: int) -> None:
    ...
    if manifest["status"] == "COMPLETE":
        print(experiment); print(experiment.with_suffix(".zip")); return
    manifest["status"] = "RUNNING"; _write(experiment / "manifest.json", manifest)
    names = [f"replicate_{r}/{case}_{method}" for r in REPLICATES for _label, case in CASES for method in METHODS]
    commands = [[sys.executable, "-B", str(root / "scripts/run_phase3c_shared_scenario_final.py"),
                 "--experiment-dir", str(experiment), "--trajectory", name] for name in names]
    ...
    for request in requests:
        ...
        if not (experiment / "assessment/cells" / request["cell_id"] / "result.json").exists():
            tasks.append(([sys.executable, "-B", str(root / "scripts/run_phase3c_shared_scenario_final.py"),
                           "--experiment-dir", str(experiment), "--assessment-worker", str(path)], request["cell_id"]))
    with ThreadPoolExecutor(max_workers=assessment_workers) as pool:
        futures = {pool.submit(_run_subprocess, command, experiment / "assessment/cells" / cell / "assessment.log"): cell
                   for command, cell in tasks}
        for future in as_completed(futures):
            code, log = future.result()
            if code: raise RuntimeError(f"Assessment {futures[future]} failed; inspect {log}")
    build_final_outputs(root, experiment)
    manifest = _read(experiment / "manifest.json"); manifest["status"] = "COMPLETE"
    manifest["completed_utc"] = dt.datetime.now(dt.timezone.utc).isoformat(); _write(experiment / "manifest.json", manifest)
    build_final_outputs(root, experiment, rebuild_zip=True)
    print(experiment); print(experiment.with_suffix(".zip"))
...
def validate_prepared(root: Path, experiment: Path) -> None:
    ...
    result = {"status": _read(experiment / "manifest.json")["status"],
              "optimization_calls": len(calls), "naive_calls": sum(x["method"] == "naive" for x in calls),
              "shared3_calls": sum(x["method"] == "shared3" for x in calls),
              "assessment_cells": len(assessment)}
    if (result["optimization_calls"], result["naive_calls"], result["shared3_calls"], result["assessment_cells"]) != (6884, 1604, 5280, 200):
...
def main(argv=None) -> None:
    ...
    args = parser.parse_args(argv); root = Path(__file__).resolve().parents[4]
    if args.prepare_only:
        path = create_plan(root, args.experiment_dir); freeze_contract(root, path); print(path); return
    if not args.experiment_dir: parser.error("--experiment-dir is required unless --prepare-only")
    experiment = args.experiment_dir.resolve()
    if args.smoke_only: print(json.dumps(run_smoke(root, experiment), indent=2))
    elif args.validate_only: validate_prepared(root, experiment)
    elif args.trajectory:
        result = run_named_trajectory(root, experiment, args.trajectory); print(result["status"])
        if result.get("error"): raise SystemExit(1)
    elif args.assessment_worker: assessment_worker(root, experiment, args.assessment_worker.resolve())
    else:
        if not 1 <= args.max_parallel_sa <= 4: parser.error("--max-parallel-sa must be in [1,4]")
        if not 1 <= args.assessment_workers <= 10: parser.error("--assessment-workers must be in [1,10]")
